...
function domain_mapping_plugins_uri( $full_url, $path=NULL, $plugin=NULL ) {
	...
	if ( false !== strpos($full_url, 'content/mu-plugins') )
    	$mapped_url = get_option( 'siteurl' ) . substr( $full_url, stripos( $full_url, 'content/mu-plugins' ) - 1 );
	  else
	    $mapped_url = get_option( 'siteurl' ) . substr( $full_url, stripos( $full_url, 'content/plugins' ) - 1 );
	  return $mapped_url;
}
